...
class MenuBar(tkinter.Menu):
    ...
    def help_dialog(self, event):
        ...
        PopupDialog(self, '大一统工具箱', _description)

    def about_dialog(self):
        "Dialog concerning information about entities responsible for program."

        _description = 'Great Unity Platform Toolbox'
        if _description == '':
            _description = _('No description available')
        _description += '\n'
        _description += '\n' + _('Author') + ': Great Unity Toolbox'
        _description += '\n' + _('Email') + ': nianqinianyi@163.com'
        _description += '\n' + _('Version') + ': 0.0.1'
        _description += '\n' + _('GitHub Package') + \
                        ': great-unity-toolbox'
        PopupDialog(self, _('About') + ' 大一统工具箱',
                    _description)
